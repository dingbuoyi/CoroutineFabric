...
基于 [Kotlin-Coroutine-API-Requirements-v1.1.md](./docs/Kotlin-Coroutine-API-Requirements-v1.1.md) 与 [Kotlin-Coroutine-API-Design-v1.1.md](./docs/Kotlin-Coroutine-API-Design-v1.1.md) 实现的 Android Library：
...
- 测试：JUnit 4 + `kotlinx-coroutines-test`（`runTest` 虚拟时间 + 真实线程高竞争）；
  测试范围与验收条件见 [CoroutineCoordinator-Test-Cases-v1.1.md](./docs/CoroutineCoordinator-Test-Cases-v1.1.md)
...
- **joinOnce（suspend）**：运行一次刷新（1.5s）并挂起等待其完成；“连续调用 4 次”可观察只有首个 block 真正执行，其余调用 join 当前 execution，挂起等待其完成后返回。
- **launchQueued / joinQueued（suspend）**：一次提交 A/B/C，严格 FIFO 串行（各 0.8s）；“等待方式”下每个 `joinQueued()` 在自己的执行完成后返回（而不是等整队排空）。
...
// 2) joinOnce：suspend 版本——首个调用执行 block，并发调用 join 当前 execution 并等待其完成
lifecycleScope.launch {
    coordinator.joinOnce(refreshKey) { repository.refresh() }  // 返回时 execution 已完成
}
...
//    joinQueued：suspend 版本——等待“自己的” execution 完成后返回
lifecycleScope.launch {
    coordinator.joinQueued(uploadKey) { upload(batchC) }
}
...
suspend fun CoroutineCoordinator.joinOnce(key: CoordinatorKey.Once, block: suspend CoroutineScope.() -> Unit)
fun CoroutineCoordinator.launchQueued(key: CoordinatorKey.Queued, block: suspend CoroutineScope.() -> Unit): Unit
suspend fun CoroutineCoordinator.joinQueued(key: CoordinatorKey.Queued, block: suspend CoroutineScope.() -> Unit)
...
| `joinOnce` | 同策略的 suspend 形式：首个调用执行 block；并发调用不执行自己的 block，挂起等待当前 execution 完成后返回 | 挂起直至 execution 完成（正常返回 / 重抛失败 / scope 取消时抛 `CancellationException`） |
| `launchQueued` | 只要 owning scope 保持 active：全部执行、严格 FIFO、不重叠；scope 取消后不保证剩余队列继续执行；不等待 | `Unit` |
| `joinQueued` | 同策略的 suspend 形式：等待**自己的** execution 完成（不等整队排空） | 挂起直至自己的 execution 完成（结果语义同上） |
...
`Conflate` 不再是独立策略：原“触发合并”语义由 `joinOnce` 的 suspend 形式覆盖。
...
4. **scope 取消**：所有 running/queued/pending 全部取消、不再启动，key 状态清理干净，不留下脱离 scope 的工作；
   挂起在 `joinOnce`/`joinQueued` 上的调用者收到 `CancellationException`。scope 已取消时：
   `launch*` 静默返回（不登记任何工作，带值 API 也不执行 snapshot 函数）；`joinOnce`/`joinQueued` 抛 `CancellationException`。
...
- suspend 形式的实现要点：`joinOnce` 的后续调用直接 `await` 当前 running submission 的完成门（“join 当前 execution”）；
  `joinQueued` 的调用者 `await` 自己 submission 的完成门。

### 与设计文档的对应关系

- key 参数类型为策略绑定的 sealed 子类型（`Once`/`Queued`/`Coalesced<T>`），策略混用是编译期错误；
  工厂形态 `CoordinatorKey.once()/queued()/coalesced<T>()`。
- `joinOnce`/`joinQueued` 的 suspend 契约落实为“挂起直至所参与的 execution 完成，并观察其结果”
...
测试范围、方法与验收条件以 `docs/CoroutineCoordinator-Test-Cases-v1.1.md` 为契约
（三层模型：Contract / Transition / Concurrency Robustness；
UT-O01~O08 / UT-Q01~Q06 / UT-C01~C06 / UT-D01~D02 / UT-L01 / UT-T01），
每个用例带 `// UT-xxx` 追溯注释（目录外保护性用例标注 Additional）。
全部使用虚拟时间（`CompletableDeferred` 门 + `runCurrent`/`advanceUntilIdle`，不依赖真实时间），
只断言公开可观察行为（submitted vs executed），不依赖内部状态。

测试覆盖（66 个用例，按策略分文件）：

- `CoroutineCoordinatorOnceTest`（14）：UT-O01~O08——首次执行（O01）、重复提交丢弃（含多并发重复，O02）、
  完成后重新提交（O03，at most one **active** execution）、`launchOnce` 不等待、`joinOnce` 首个调用者执行并等待
  （O04）、重复调用者 join 当前 execution（自己的 block 不执行，O05）、多 joiner（O06）、**取消一个 joiner 不取消共享
  execution（O07，P0）且不影响其他 joiner（O07）**、execution 异常传播给所有 joiner 与 scope handler 且 key 状态清理
  （O08）；另含目录外用例：已取消 scope 下 `joinOnce` 抛 CE / `launchOnce` 不登记、普通 Job scope 下失败级联取消
- `CoroutineCoordinatorQueuedTest`（19）：UT-Q01~Q06——单执行（Q01）、单/双/多提交严格 FIFO（Q02，乱序即失败）、
  永不重叠（Q03，`maxActiveExecutions` 计数器断言 ==1）、`launchQueued` 不等待（Q04）、`joinQueued` 只等自己的
  execution（Q05，不等整队，含多个调用者各自独立恢复）、scope 取消中止剩余队列并 CE 唤醒等待者（Q06）；
  另含目录外用例：**Job vs SupervisorJob 失败矩阵**（普通 Job 级联取消中止剩余队列 / supervisor 不引入额外取消且
  队列续跑）、取消非失败（队列续跑、handler 不触发，Job/Supervisor 两例）、等待者观察自己 execution 的失败
  且后续继续、失败保留原始类型/消息且 key 可重新提交、已取消 scope 行为、排空后重启
- `CoroutineCoordinatorCoalescedTest`（15）：UT-C01~C06——单提交立即执行（C01）、pending 在 running 后执行、
  只执行 latest（B/C 从未执行）且 **running 不被新提交取消（C02/C03）**、永不重叠、**提交时不可变 snapshot
  （C04：可变输入提交后修改不可见、被覆盖时快照仍按提交点冻结、每次提交即时执行 snapshot、默认透传）**、
  **commitment point：提升后的 pending 不可再被覆盖（C05，A → D → E 而非 A → E）**、
  scope 取消丢弃 latest pending（C06）；另含目录外用例：失败/取消下 pending 接力（supervisor）、
  普通 Job scope 下失败中止 latest pending、已取消 scope 不登记且不执行 snapshot
- `CoroutineCoordinatorDomainTest`（11）：UT-D01~D02 / UT-L01 / UT-T01——同 coordinator 异 key 独立
  （D01：key 是对象身份，两个 `once()` 实例互不影响，once/queued 两例）、**异 coordinator 同 key 独立
  （D02：同 scope 两例 + 异 scope 一例：coordinator 实例身份 = 协调域）**、UT-L01——scope 取消后所有 key 状态清零
  且无孤儿协程、coordinator 无自有 Job（scope 是唯一生命周期杠杆）；UT-T01——策略混用编译期拒绝（子类型不兼容，
  第一阶段依赖 Kotlin 编译器验证）；另含同 coordinator 同 key 协调（目录基础前提）、worker 运行在 coordinator
  scope 的 context 上
- `CoroutineCoordinatorRaceConditionTest`（7）：v1.1 三层模型的 Layer 2 / Layer 3 回归——Transition
  （finish ↔ submit 完成边界提交不丢失）+ Concurrency Robustness（32 并发提交不丢失不重复不重叠；
  真实线程高竞争：`launchOnce` 风暴恰好一个、FIFO 无丢失无并行、完成/提交竞态无丢失、`joinOnce` 风暴永不重叠），
  每个压力用例重复 10–20 轮。真实线程压力测试定位为实现健壮性回归保护：跨线程提交安全不属于
  公开 API 契约（严格 FIFO 与 latest-wins 由 Layer 1 契约测试验证）
